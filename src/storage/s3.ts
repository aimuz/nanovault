...
import type { IBlobStorage, IBlobObject } from './interfaces'
...
export const getCipher = async (bucket: IBlobStorage, userId: string, cipherId: string): Promise<Cipher | null> => {
    const obj = await bucket.get(cipherKey(userId, cipherId))
    if (!obj) return null
    return await obj.json() as Cipher
}

export const putCipher = async (bucket: IBlobStorage, userId: string, cipher: Cipher): Promise<void> => {
    await bucket.put(cipherKey(userId, cipher.id), JSON.stringify(cipher), {
        httpMetadata: { contentType: 'application/json' }
    })
}

export const deleteCipher = async (bucket: IBlobStorage, userId: string, cipherId: string): Promise<void> => {
    await bucket.delete(cipherKey(userId, cipherId))
}

export const listCiphers = async (bucket: IBlobStorage, userId: string): Promise<Cipher[]> => {
...
export const getFolder = async (bucket: IBlobStorage, userId: string, folderId: string): Promise<Folder | null> => {
    const obj = await bucket.get(folderKey(userId, folderId))
    if (!obj) return null
    return await obj.json() as Folder
}

export const putFolder = async (bucket: IBlobStorage, userId: string, folder: Folder): Promise<void> => {
    await bucket.put(folderKey(userId, folder.id), JSON.stringify(folder), {
        httpMetadata: { contentType: 'application/json' }
    })
}

export const deleteFolder = async (bucket: IBlobStorage, userId: string, folderId: string): Promise<void> => {
    await bucket.delete(folderKey(userId, folderId))
}

export const listFolders = async (bucket: IBlobStorage, userId: string): Promise<Folder[]> => {
...
export const getAttachment = async (bucket: IBlobStorage, userId: string, cipherId: string, attachmentId: string): Promise<IBlobObject | null> => {
...
export const deleteAttachment = async (bucket: IBlobStorage, userId: string, cipherId: string, attachmentId: string): Promise<void> => {
    await bucket.delete(attachmentKey(userId, cipherId, attachmentId))
}

// Delete all attachments for a cipher
export const deleteAllAttachments = async (bucket: IBlobStorage, userId: string, cipherId: string): Promise<void> => {
...
export const deleteVault = async (bucket: IBlobStorage, userId: string): Promise<void> => {
